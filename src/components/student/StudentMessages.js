...
const StudentMessages = () => {
  ...
  const fetchProfile = async (uid) => {
    ...
    if (error) {
      ...
      setStudentName(data.full_name || 'Siswa');
  ...
  // ─── Deteksi query param dari StudentHome ─────────────────────────────
  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const guruId = params.get('guruId');
    const guruNama = params.get('guruNama');
    if (guruId) {
      setReceiverType('teacher');
      setSelectedTeacherId(guruId);
      setNewSubject(`Pesan untuk ${guruNama || 'Guru'}`);
      setActiveTab('new');
    }
  }, [location.search]);
  ...
  const sendNewMessage = async () => {
    ...
    if (receiverType === 'teacher' && !selectedTeacherId) {
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
                  ...
                  <tbody>
                    {filteredMessages.map(item => {
                      const isFromAdmin = !item.is_from_student;
      ...
      {activeTab === 'new' && (
        <div style={{ background: C.white, borderRadius: '24px', padding: '2rem', boxShadow: '0 4px 16px rgba(0,0,0,0.06)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div>
              <label style={{ display: 'block', fontWeight: 'bold', marginBottom: '6px', color: C.dark }}>Kirim ke</label>
              <select
                value={receiverType}
                onChange={e => {
            ...
            {receiverType === 'teacher' && (
              <div>
                <label style={{ display: 'block', fontWeight: 'bold', marginBottom: '6px', color: C.dark }}>Pilih Guru</label>
                <select
                ...
                >
                  ...
                  {teachers.map(guru => (
                    <option key={guru.id} value={guru.id}>{guru.full_name}</option>
                  ))}
                </select>
              </div>
            )}
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', marginBottom: '1rem' }}>
              ...
              <div><strong>Pengirim:</strong> {selectedMessage.is_from_student ? 'Saya' : 'Admin'}</div>
            ...
            {!selectedMessage.is_from_student && (
              <div>
